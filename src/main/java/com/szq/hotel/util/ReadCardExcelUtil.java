                ...
                if (null != cell) {
                    if (c == 0) {
                        ...
                        if (cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC) {
                            ...
                            map.put("cardNumber", name.substring(0, name.length() - 2 > 0 ? name.length() - 2 : 1));// 名称
                        } else {
                            map.put("cardNumber", cell.getStringCellValue());// 名称
                        }
                    } else if (c == 1) {
                        if (cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC) {
                            String sex = String.valueOf(cell.getNumericCellValue());
                            map.put("money",sex.substring(0, sex.length() - 2 > 0 ? sex.length() - 2 : 1));// 性别
                        } else {
                            map.put("money",cell.getStringCellValue());// 性别
                        }
                    } else if (c == 2) {
                        if (cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC) {
